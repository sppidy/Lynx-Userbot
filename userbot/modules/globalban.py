...
async def get_full_user(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        if user.isnumeric():
            user = int(user)
        if not user:
            await event.edit("`Mohon Maaf, Ini Tidak Mungkin Tanpa ID Pengguna`")
            return
...
async def gben(userbot):
    ...
    await dark.edit(f"`Global Banned Akan Segera Aktif, Anda Akan Dibanned Secara Global Oleh Saya ヅ`")
    ...
    if user:
        ...
        await dark.edit(f"`Mohon Balas Ke Pesan Yang Mulia`")
    try:
...
async def gunben(userbot):
    ...
    if user:
        if user.id == 1448477501:
            return await dark.edit("**Pengguna Tidak Bisa Terkena Perintah Ini, Karna Dia Adalah Pembuatku ヅ**")
        try:
        ...
        await dark.edit("`Harap Balas Ke Pesan Pengguna Yang Mulia ヅ`")
    try:
