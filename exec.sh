...
if [ -z ${1} ] || [ ${1} == "-x" ] ; then
	...
	cd ${BOTDIR}
	. venv/bin/activate
	nohup python3 -m userbot >>${UNIXTIME}-lynxuserbot.log 2>&1 & echo "I: Userbot PID is ${!}"
	deactivate
	cd ${RUNDIR}
	exit 0
	...
	echo -e "--daemonize    || -d -- Run in the background, logs will be saved in *-lynxuserbot.log"
	exit 0
else
	echo -e  "Lynx-Userbot"
        echo -e  "Licensed under Raphielscape Public License"
